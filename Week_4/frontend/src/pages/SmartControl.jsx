...
export default function SmartControl() {
  const { data: rawDevices, loading } = usePoll(api.getDevices, 3000)
  const [filter, setFilter] = useState('all')
  const [localOverrides, setLocalOverrides] = useState({})

  // Merge polled data with local optimistic overrides
  const devices = rawDevices?.map(d =>
    localOverrides[d.id] !== undefined
      ? { ...d, is_on: localOverrides[d.id] }
      : d
  )

  const handleToggle = async (id, is_on) => {
    // Optimistic update — toggle reflects instantly in UI
    setLocalOverrides(prev => ({ ...prev, [id]: is_on }))
    try {
